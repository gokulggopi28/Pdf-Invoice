...
class PdfInvoiceService {
  ...
  Future<Uint8List> createInvoice(List<Product> soldProducts) async {
    ...
    final List<CustomRow> elements = [
      CustomRow("Item Name", "Item Price", "Amount", "Total", "Gst"),
      for (var product in soldProducts)
        CustomRow(
          ...
          (product.gstInPercent * product.price).toStringAsFixed(2),
        ),
      CustomRow(
        "Sub Total",
        "",
      ...
      CustomRow(
        ...
        "",
        "",
        "${(double.parse(getSubTotal(soldProducts)) + double.parse(getGstTotal(soldProducts))).toStringAsFixed(2)} EUR",
      )
    ];
  ...
  String getGstTotal(List<Product> products) {
    return products
        .fold(
          0.0,
          (double prev, next) =>
              prev + ((next.price / 100 * next.gstInPercent) * next.amount),
        )
        .toStringAsFixed(2);
  }
}